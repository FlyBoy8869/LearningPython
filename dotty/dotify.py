...
def dotify_list(list_to_dotify: list):
    ...
    for index, element in enumerate(list_to_dotify):
        if isinstance(element, dict):
            ...
            setattr(namespace, f"e{index}", ns)
        elif isinstance(element, list):
            ns = dotify_list(element)
            setattr(namespace, f"e{index}", ns)
        else:
            setattr(namespace, f"e{index}", element)
...
def dotify_dictionary(dict_to_dotify: dict):
    ...
    for key, value in dict_to_dotify.items():
        if isinstance(value, dict):
            ...
            # list_namespace = SimpleNamespace()
            # for index, element in enumerate(value):
            #     if isinstance(element, dict):
            #         ns = dotify_dictionary(element)
            #         setattr(list_namespace, f"e{index}", ns)
            #     else:
            #         setattr(list_namespace, f"e{index}", element)
            ns = dotify_list(value)
            # setattr(namespace, key, list_namespace)
            setattr(namespace, key, ns)
        else:
...
def dotify_get_namespace_items(namespace: SimpleNamespace):
    return tuple([member for member in namespace.__dict__ if not member.startswith("_")])
...
class DottedAssembly:
    ...
    def assembly_from_json_object(cls, json_object):
        djson = dotify_dictionary(json_object)
        return cls(
            title=djson.title,
